...
### Viewers & Tools
- **viewer.py** — main hyperspectral BSQ viewer (tkinter + matplotlib). Displays RGB preview of the data cube, click any pixel to inspect its full spectral signature, export to CSV.
- **viewer_with_area_types.py** — extended viewer with spectral library building tools. Draw rectangles over land cover areas, extract per-class spectral statistics (mean, std, min, max) using streaming Welford algorithm, export to library CSV.
- **plot_library.py** — standalone script to visualise spectral library CSV files. Plots mean spectrum with ±1 std shaded envelope for one or multiple land cover classes.

### Water Quality
- **water_quality_indices.py** — calculates water quality indices (Chl-a, DOC, turbidity) from the hyperspectral cube and Sentinel-2 L2A bands. Produces side-by-side comparison maps and a summary CSV.
